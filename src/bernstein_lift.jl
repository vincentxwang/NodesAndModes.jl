...
"""
function fast!(out, ::ElevationMatrix{N}, L0, x, offsets) where {N}
    L0x = L0 * x # note L0 is very very sparse
    index1 = div((N + 1) * (N + 2), 2)
    out[1:index1] = L0x
    E = fast!(zeros(div((N) * (N+1), 2)), ReductionMatrix{N}(), L0x, offsets[N])
    for j in 1:N
        ...
        index1 = index2
        if j < N
            E = fast!(E, ReductionMatrix{N-j}(), E, offsets[N-j])
        end
...
@btime fast!($(out), $(ElevationMatrix{7}()), L0, x, $(offset_table))
@btime $(lift(7)) * $(x)
...
@code_warntype fast!(out, ElevationMatrix{7}(), x, L0, offset_table)
...
@test fast!(out, ElevationMatrix{7}(), L0, x, offset_table) ≈ lift(7) * x
...
@btime index1 = div((N + 1) * (N + 2), 2)
@btime L0 * x
L0x = L0 * x
@btime out[1:index1] = L0x
@btime fast!($(zeros(div((N) * (N+1), 2))), ReductionMatrix{N}(), $(L0x), $(offse[N]))
@btime binomial(7,3)


fast!(out, ElevationMatrix{7}(), L0 * x, offset_table)
@profile fast!(out, ElevationMatrix{7}(), L0, x, offset_table)
Profile.print()

@report_opt fast!(out, ElevationMatrix{7}(), L0, x, offset_table)
