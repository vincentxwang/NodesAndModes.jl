"""
run bernstein.jl before running this.
"""
using Test
using LinearAlgebra
...
function bernstein_2d_matrix(N::Int)
    scalar_to_multiindex, multiindex_to_scalar = bernstein_2d_scalar_multiindex_lookup(N)
    Np = length(scalar_to_multiindex)
    D = zeros(Int, Np, Np)
    for index in 1:Np
        (i,j,k) = scalar_to_multiindex[index]
        D[index, index] = i
        if j >= 1 D[index, multiindex_to_scalar[(i+1,j-1,k)]] = j end
        if k >= 1 D[index, multiindex_to_scalar[(i+1,j,k-1)]] = k end
    end
    return sparse(D)
end

bernstein_2d_matrix(3)
...
@testset "2D Bernstein derivative verification" begin
    @test evaluate_bernstein_derivative_matrices(Tri(), 3)[1] ≈ bernstein_2d_matrix(3)
    ...
    @test evaluate_bernstein_derivative_matrices(Tri(), 5)[1] ≈ bernstein_2d_matrix(5)
