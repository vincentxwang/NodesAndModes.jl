...
"""
function fast!(out, ::ElevationMatrix{N}, L0, x, offset) where {N}
    E = L0 * x # note L0 is very very sparse, could speedup later
    index1 = div((N + 1) * (N + 2), 2)
    out[1:index1] = E
    E = reduction_multiply!(E, ReductionMatrix{N}(), E, offset[N])
    for j in 1:N
...
@btime fast!($(out), $(ElevationMatrix{N}()), $(L0), $(x), $(offset_table))
@btime mul!($(out), $(lift(N)), $(x))

@test fast!(out, ElevationMatrix{N}(), L0, x, offset_table) ≈ lift(N) * x
