...
"""
struct BernsteinDerivativeMatrix_2D_r{N} <: AbstractMatrix{Float64} end
struct BernsteinDerivativeMatrix_2D_s{N} <: AbstractMatrix{Float64} end
...
"""
    compute_derivative_r!(out, ::BernsteinDerivativeMatrix_2D_r{N}, x, offset) where {N}

Computes the derivative with respect to the r coordinate and stores the result in the output array.

# Parameters
- `out`: The output array to store the result.
- `x`: The input array.
...
N=7
@btime fast!($(copy(b_7)), $(BernsteinDerivativeMatrix_2D_r{N}()), $x_7, $(offsets(7)))
@btime mul!($(copy(b_7)), $(Matrix(BernsteinDerivativeMatrix_2D_r{7}())), $x_7)
...
@testset "r/s matricies vs. i/j/k matrices" begin
    ...
    x_7 = rand(Float64, div((7 + 1) * (7 + 2), 2))
    b_7 = similar(x_7)
    @test 0.5 * (LinearAlgebra.mul!(copy(b_7), evaluate_bernstein_derivative_matrices(Tri(), 7)[1], x_7) - mul!(copy(b_7), evaluate_bernstein_derivative_matrices(Tri(), 7)[3], x_7)) ≈ fast!(copy(b_7), BernsteinDerivativeMatrix_2D_r{7}(), x_7)
    @test 0.5 * (LinearAlgebra.mul!(copy(b_7), evaluate_bernstein_derivative_matrices(Tri(), 7)[2], x_7) - mul!(copy(b_7), evaluate_bernstein_derivative_matrices(Tri(), 7)[3], x_7)) ≈ fast!(copy(b_7), BernsteinDerivativeMatrix_2D_s{7}(), x_7)
end
